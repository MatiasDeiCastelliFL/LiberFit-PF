...
const TrainingsForms = ({background}:Props) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col items-center w-2/3 border rounded-l-custom_1 p-4  gap-3">
            ...
            <input type="number" name="muculo" value={data.precio} onChange={handleChange} placeholder='músculo' className="px-4 font-sans text-xl border border-cyan-600 rounded-full font-light w-full text-gray-500"/>
            <select name="ejercicios" value={data.profesores} onChange={handleSelect} className="px-4 font-sans text-xl border border-cyan-600 rounded-full font-light w-full text-gray-500">
